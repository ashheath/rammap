 ...
 */
export const getVehicles = async (gridCell?: string): Promise<Vehicle[]> => {
 ...
 * Get vehicle by VRM
 */
export const getVehicleByVRM = async (vrm: string): Promise<Vehicle | undefined> => {
  const result = await pool.query(
    `SELECT * FROM vehicles WHERE vrm = $1 LIMIT 1`,
    [vrm.toUpperCase()]
  )
 ...
 */
export const saveVehicle = async (vehicle: Omit<Vehicle, 'id' | 'created_at' | 'updated_at'>): Promise<Vehicle> => {
 ...
 */
export const deleteVehicle = async (vrm: string): Promise<boolean> => {
